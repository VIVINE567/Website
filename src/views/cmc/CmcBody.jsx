...
import { TYPE_CARDS, SPEC_TABLE, GRADES } from '../../data/cmcSpecs';
import { RANK, REG_BADGES, REF_COLS, REF_TABLE, REF_NOTE } from '../../data/cmcApps';
import { APP_DETAILS, SEO_TITLE, SEO_PARAS, CHART_FOOTER_NOTE, FOOTER_NOTE } from '../../data/cmcDetails';
...
const MarketRank = () => (
  <section>
    <SectionHeader title="Industry application ranking — Sodium CMC" sub="By volume and commercial significance across 8 industries" />
...
const InfoCards = () => (
  <section>
    <SectionHeader title="Key properties of Sodium CMC" sub="What makes it unique across 8 industries" />
...
/* ── 4. DS / Purity Type Banner ── */
const TypeBanner = () => (
  <section>
    <SectionHeader title="Degree of substitution (DS) classes & purity grades" />
...
/* ── 5. Standard Specification Table (with section headers) ── */
const SpecTable = () => (
  <section>
    <SectionHeader title="Standard specification" sub="Typical parameters — all grades (food, pharma, technical)" />
          ...
          {SPEC_TABLE.map((row, i) => {
            ...
            return (
              <tr key={`r-${i}`} style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
                ...
                <td className="px-4 py-2.5" style={{ fontFamily: "'Cormorant Garamond', serif", color: 'var(--forest)', fontWeight: 600, fontSize: '0.98rem' }}>{row.v}</td>
              </tr>
            );
          })}
        </tbody>
...
/* ── 8. Viscosity Chart (logarithmic, 9 bars, pointer-following tooltip) ── */
const ViscChart = () => {
  ...
  return (
    <section>
      <SectionHeader title="Viscosity grade guide by application" sub="Brookfield 1% aqueous, 25°C — indicative ranges (mPa·s)" />
      <motion.div {...fadeUp} className="rounded-xl border p-5 md:p-6" style={{ background: 'linear-gradient(170deg, var(--cream) 0%, var(--cream-dark) 100%)', borderColor: 'rgba(201,168,76,0.25)' }}>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1.5 mb-4">
          {bars.map((b) => (
            <span key={b.id} className="flex items-center gap-1.5 text-[10.5px]" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>
              <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: b.color }} />{b.label} mPa·s
            </span>
          ))}
        </div>
          ...
          <div className="flex-1 min-w-0">
            <div ref={plotRef} className="relative" style={{ height: 280 }} onMouseLeave={() => setTip(null)}>
              ...
              {tip && (
                ...
                >
                  <div className="text-[12.5px] mb-1" style={{ fontFamily: "'Cormorant Garamond', serif", color: 'var(--gold-light)', fontWeight: 700 }}>{tip.b.label} mPa·s</div>
              ...
              <div className="absolute left-0 right-0 flex items-end justify-around gap-1" style={{ bottom: 28, top: 0 }}>
                {bars.map((b) => (
                  <div key={b.id} className="flex flex-col items-center justify-end flex-1 h-full">
                    <motion.div
                      initial={{ height: 0 }}
              ...
              <div className="absolute left-0 right-0 bottom-0 flex items-start justify-around gap-1" style={{ height: 28 }}>
                {bars.map((b) => (
                  <div key={b.id} className="flex-1 text-center leading-tight pt-1.5 text-[8.5px] font-semibold" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--forest)' }}>{b.label}</div>
                ))}
              </div>
            </div>
          </div>
...
const ChartFooterNote = () => (
  <div className="rounded-xl border p-3.5 flex items-start gap-2.5 text-[12.5px] leading-relaxed" style={{ background: 'rgba(201,168,76,0.08)', borderColor: 'rgba(201,168,76,0.3)', color: 'var(--gold-dark)', fontFamily: "'Raleway', sans-serif" }}>
    ...
    <div>{CHART_FOOTER_NOTE}</div>
  </div>
);

/* ── 10. Application Reference Table (16 rows) ── */
const AppRefTable = () => (
  <section>
...
/* ── 11. Application Deep-Dive ── */
const AppDeepDive = () => (
  <section>
    <SectionHeader title="Industry application deep-dive" sub="Detailed use cases across all 8 industries" />
