...
import { IC, HIGHLIGHTS, INFO_CARDS, VISC_CHART } from '../../data/pacContent';
import { SPEC_TABLE, GRADES } from '../../data/pacSpecs';
import { REGULATORY, REF_COLS, REF_TABLE, APP_DETAILS, COMPARE } from '../../data/pacApps';
import { SEO_TITLE, SEO_PARAS, FOOTER_NOTE } from '../../data/pacDetails';
...
const FILTERS = [
  ...
  { key: 'textile', id: 'textile', label: 'Textile' },
...
const InfoCards = () => (
  <section>
    <SectionHeader title="Key properties of PAC" sub="What sets it apart from standard CMC" />
...
const Regulatory = () => (
  <section>
    <SectionHeader title="Standards & certifications" />
        ...
        <tbody>
          {REF_TABLE.map((r) => (
            <tr key={r.grade} style={{ borderTop: '1px solid rgba(201,168,76,0.12)', verticalAlign: 'top' }}>
              ...
              <td className="px-3.5 py-2.5 whitespace-nowrap" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>{r.ds}</td>
              <td className="px-3.5 py-2.5 whitespace-nowrap" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>{r.purity}</td>
              <td className="px-3.5 py-2.5 whitespace-nowrap" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>{r.apiFl}</td>
              <td className="px-3.5 py-2.5 whitespace-nowrap" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>{r.acid}</td>
              <td className="px-3.5 py-2.5" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)', lineHeight: 1.5, maxWidth: 260 }}>{r.func}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const Comparison = () => (
  <section>
    <SectionHeader title="PAC vs. Standard CMC — performance comparison" />
    <motion.div {...fadeUp} className="rounded-md border overflow-x-auto" style={{ background: 'var(--cream)', borderColor: 'rgba(201,168,76,0.25)' }}>
      <table className="w-full text-[13px]" style={{ borderCollapse: 'collapse', minWidth: 700 }}>
        <thead>
          <tr style={{ background: 'rgba(201,168,76,0.1)' }}>
            {COMPARE.cols.map((h) => (
      ...
      {COMPARE.note && (
        <div className="px-3.5 py-2.5 text-[11.5px] leading-relaxed" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)', borderTop: '1px solid rgba(201,168,76,0.12)', background: 'rgba(201,168,76,0.04)' }}>
          {COMPARE.note}
        </div>
      )}
    </motion.div>
...
export default function PacBody() {
  return (
