...
import { HERO } from '../data/alginateContent';
import AlginateBody from './alginate/AlginateBody';
...
export default function ProductDetailAlginate({ slug }) {
    ...
    <div className="pt-[72px]" style={{ background: 'var(--cream-dark)' }}>
      ...
      <section className="relative pb-12 pt-20 px-4 md:px-6 overflow-hidden" style={S.productDetailHero}>
        ...
        <div
          ...
          aria-hidden="true"
          style={{ right: '-1rem', top: '1.5rem', fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: '7rem', lineHeight: 1, color: 'rgba(232,201,122,0.06)', letterSpacing: '0.05em' }}
        >
          E401
        </div>
        <div className="relative max-w-7xl mx-auto w-full">
          <motion.div {...fadeUp(0)} className="flex items-center gap-2 mb-6 uppercase" style={S.productDetailBreadcrumb}>
            ...
            <ChevronRight className="w-3 h-3" style={{ opacity: 0.5 }} />
            <Link href="/products" className="hover:opacity-80 transition-opacity">Hydrocolloids & Gums</Link>
            <ChevronRight className="w-3 h-3" style={{ opacity: 0.5 }} />
            <span>Sodium Alginate</span>
          </motion.div>
