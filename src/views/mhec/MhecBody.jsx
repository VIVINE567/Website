...
import { IC, HIGHLIGHTS, TYPE_COMPARE, SOURCING_NOTE, INFO_CARDS } from '../../data/mhecContent';
import { SPEC_TABLE, GRADES, VISC_CHART } from '../../data/mhecSpecs';
import { APP_TABLE } from '../../data/mhecApps';
import { APP_DETAILS, REGULATORY, SEO_TITLE, SEO_PARAS, FOOTER_NOTE } from '../../data/mhecDetails';
...
const FILTERS = [
  ...
  { id: 'tile', label: 'Tile Adhesive' },
  { id: 'putty', label: 'Wall Putty' },
  { id: 'skim', label: 'Skim Coat' },
  { id: 'gypsum', label: 'Gypsum Plaster' },
  { id: 'drymix', label: 'Dry-Mix Mortar' },
...
const TypeCompare = () => (
  <section>
    <SectionHeader title="MHEC vs MHEC-RD — understanding the nomenclature" sub="Same base chemistry, different designation conventions" />
        ...
        <motion.div key={c.name} {...fadeUp} className="rounded-md p-4 border" style={{ background: c.color.bg, borderColor: c.color.border, color: c.color.tx }}>
          ...
          {c.props.map(([k, v]) => (
            <div key={k} className="flex gap-1.5 text-[11.5px] mb-1" style={{ fontFamily: "'Raleway', sans-serif" }}>
              <span style={{ opacity: 0.65, flexShrink: 0, width: 120 }}>{k}</span>
...
const SourcingNote = () => (
  <motion.div {...fadeUp} className="rounded-md border p-5" style={{ background: '#FFFBEC', borderColor: '#E8D888' }}>
    <h3 className="mb-2" style={{ fontFamily: "'Raleway', sans-serif", fontSize: '0.875rem', fontWeight: 600, color: '#7A5800' }}>{SOURCING_NOTE.title}</h3>
    <p className="text-[12.5px] leading-relaxed" style={{ fontFamily: "'Raleway', sans-serif", color: '#6A4A00' }} dangerouslySetInnerHTML={{ __html: SOURCING_NOTE.body }} />
  </motion.div>
);

const InfoCards = () => (
  <section>
    <SectionHeader title="Chemical identity & physical properties" sub="Standard reference data" />
...
const SpecsAndGrades = () => {
  ...
  return (
    <>
      <section>
        <SectionHeader title="Standard technical specification" sub="Reference values — actual TDS varies by grade and manufacturer" />
      ...
      <section>
        <SectionHeader title="Grade comparison" sub="Select an application segment to highlight relevant grades" />
    ...
    <section>
      ...
      <motion.div {...fadeUp} className="rounded-md border p-5 md:p-6" style={{ background: 'var(--cream)', borderColor: 'rgba(201,168,76,0.25)' }}>
        ...
        <div className="flex gap-2">
          <div className="relative pb-9 text-[10px] shrink-0" style={{ height: 280, width: 30, fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)' }}>
            {ticks.map((t) => (
              ...
              <div className="absolute left-0 right-0 bottom-0 flex items-start justify-around gap-1 sm:gap-1.5" style={{ height: 38 }}>
                {VISC_CHART.bars.map((b) => (
                  <div key={b.label} className="flex-1 min-w-0 text-center leading-tight pt-1.5 text-[8px] sm:text-[9.5px] font-semibold break-words" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--forest)' }}>
                    {b.label.split('\n').map((line, li) => (<span key={li}>{line}{li === 0 ? <br /> : null}</span>))}
                  </div>
                ))}
              </div>
            </div>
...
const AppsTable = () => (
  <section>
    <SectionHeader title="Applications by industry segment" sub="Functional role and dosage range" />
      ...
      <table className="w-full text-[12.5px]" style={{ borderCollapse: 'collapse', minWidth: 900 }}>
        <thead>
          <tr style={{ background: 'rgba(201,168,76,0.1)' }}>
            {['Industry / Segment', 'Typical products', 'Recommended viscosity grade', 'Function of MHEC', 'Dosage (% w/w dry)'].map((h) => (
...
const AppDetails = () => (
  <section>
    <div className="mb-6">
      <h2 style={S.richSectionHeader}>Construction applications — detailed functional roles</h2>
    </div>
...
const Regulatory = () => (
  <section>
    <div className="mb-6">
      <h2 style={S.richSectionHeader}>Standards & regulatory references</h2>
    </div>
...
export default function MhecBody() {
  return (
