...
import { IC, HIGHLIGHTS, INFO_CARDS, DE_CHART } from '../../data/pectinContent';
import { SPEC_TABLE, GRADES, REF_TABLE } from '../../data/pectinSpecs';
import { REGULATORY, TYPE_COMPARE, APP_DETAILS } from '../../data/pectinApps';
import { SEO_TITLE, SEO_PARAS, FOOTER_NOTE } from '../../data/pectinDetails';
...
const FILTERS = [
  ...
  { id: 'jam', label: 'Jams & Jellies' },
  { id: 'dairy', label: 'Dairy' },
  { id: 'beverage', label: 'Beverages' },
  { id: 'confectionery', label: 'Confectionery' },
  { id: 'bakery', label: 'Bakery' },
  { id: 'lowsugar', label: 'Low-sugar / Diet' },
];
...
const Regulatory = () => (
  <section>
    <SectionHeader title="Regulatory approvals" sub="All grades comply with the following standards" />
...
const InfoCards = () => (
  <section>
    <SectionHeader title="Key properties of pectin" sub="Understanding DE, gelation mechanism, and type selection" />
...
const Grades = () => {
  ...
  return (
    <section>
      <SectionHeader title="Grade comparison" sub="Select a food segment to highlight relevant pectin types" />
...
const TypeCompare = () => (
  <section>
    <SectionHeader title="HM vs LM vs LMA — key differences" sub="Choosing the right pectin type for your formulation" />
...
const DeChart = () => {
  ...
  return (
    <section>
      <SectionHeader title="Degree of esterification by grade" sub="DE range comparison across all pectin types (indicative)" />
        ...
        <div className="flex gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div ref={plotRef} className="relative" style={{ height: PLOT_H }} onMouseLeave={() => setTip(null)}>
              ...
              {/* bars (floating ranges from min to max) */}
              <div className="absolute left-0 right-0 flex items-end justify-around gap-2" style={{ bottom: X_AXIS, top: 0 }}>
                {C.bars.map((b) => (
                  <div key={b.label} className="flex flex-col items-center justify-end flex-1 h-full">
                    <div
                      className="w-full flex flex-col justify-start cursor-pointer"
                      style={{ height: `${frac(b.max) * 100}%`, maxWidth: 64 }}
                      onMouseMove={onBarMove(b)}
                    >
                      {/* solid segment: min → max */}
                      <motion.div
                        initial={{ height: 0 }}
                        whileInView={{ height: `${((b.max - b.min) / b.max) * 100}%` }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
                        className="w-full rounded-t"
                        style={{ background: b.color }}
                      />
                      {/* faded base segment: 0 → min (matches HTML color + '55') */}
                      <div className="w-full" style={{ height: `${(b.min / b.max) * 100}%`, background: `${b.color}55` }} />
                    </div>
                  </div>
                ))}
              </div>
...
const RefTable = () => (
  <section>
    <SectionHeader title="Quick reference table" sub="All pectin grades at a glance" />
    <motion.div {...fadeUp} className="rounded-md border overflow-x-auto" style={{ background: 'var(--cream)', borderColor: 'rgba(201,168,76,0.25)' }}>
        ...
        <tbody>
          {REF_TABLE.rows.map((row, i) => (
            <tr key={i} style={{ borderTop: '1px solid rgba(201,168,76,0.12)', verticalAlign: 'top' }}>
              ...
              <td className="px-3.5 py-2.5" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--brown-warm)', lineHeight: 1.5 }}>{row.requirement}</td>
              ...
              <td className="px-3.5 py-2.5 whitespace-nowrap" style={{ fontFamily: "'Raleway', sans-serif", color: 'var(--forest)', fontWeight: 600 }}>{row.eno}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function PectinBody() {
  return (
